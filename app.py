...
# Runs fully in-browser. Keep index.html loading this file via <py-script src="app.py">.
...
# Current design (set after successful run)
current = None

# Keep JS proxies alive (prevents "borrowed proxy destroyed" errors)
_proxies = []
...
def render_to_imgs(figs) -> None:
    ...
    # show grid when we have content
...
def update_tooltip(*_) -> None:
    try:
        ...
        tip = window_tooltips.get(label, "")
        document.getElementById("wtip").innerText = tip
    except Exception as e:
...
def design(event=None) -> None:
    ...
    try:
        ...
        nyq = fs / 2
        if highcut >= nyq or lowcut >= nyq:
            raise ValueError(f"Cutoffs must be < Nyquist ({nyq} Hz).")
        ...
        # (1) Impulse/Step
        f1, a1 = plt.subplots(figsize=(5, 3))
        ...
        # (2) Window (time)
        f2, a2 = plt.subplots(figsize=(5, 3))
        f2.patch.set_facecolor("white")
        a2.plot(t, window_vals)
        a2.set_title(f"{wlabel} Window")
        ...
        f3, a3 = plt.subplots(figsize=(5, 3))
        ...
        a3.set_ylabel("Magnitude (dB)")
        a3.set_ylim([-80, 5])
        a3.grid(True, alpha=0.3)
        if ftype == "Lowpass":
            a3.axvline(x=highcut, linestyle="--", alpha=0.7)
        elif ftype == "Highpass":
            a3.axvline(x=lowcut, linestyle="--", alpha=0.7)
        else:
            a3.axvline(x=lowcut, linestyle="--", alpha=0.7)
            a3.axvline(x=highcut, linestyle="--", alpha=0.7)
        if document.getElementById("showPhase").checked:
            ...
            a3b.plot(w, phase, linestyle="--", alpha=0.6)
            a3b.set_ylabel("Phase (rad)")
        figs.append(f3)

        # (4) Window frequency response
        f4, a4 = plt.subplots(figsize=(5, 3))
        f4.patch.set_facecolor("white")
        a4.plot(w_win, 20 * np.log10(np.abs(h_window) + 1e-10))
        a4.set_title("Window Function Frequency Response")
        a4.set_xlabel("Frequency (Hz)")
        a4.set_ylabel("Magnitude (dB)")
        a4.set_ylim([-100, 10])
        a4.grid(True, alpha=0.3)
        figs.append(f4)

        # (5) Example signal
        f5, a5 = plt.subplots(figsize=(5, 3))
        f5.patch.set_facecolor("white")
...
def export_csv(event=None) -> None:
    ...
    # coefficients CSV
    lines = ["index,coefficient"]
    for i, c in enumerate(current["taps"]):
        lines.append(f"{i},{c}")
    csv_bytes = ("\n".join(lines) + "\n").encode("utf-8")
    download_bytes("filter_coefficients.csv", csv_bytes, "text/csv")
...
def export_plots(event=None) -> None:
    ...
    if not PIL_AVAILABLE:
        # Fallback: export each figure separately
        for i, fig in enumerate(current["figs"], start=1):
            buf = BytesIO()
            fig.savefig(buf, format="png", dpi=140, bbox_inches="tight", facecolor="white")
            buf.seek(0)
            download_bytes(f"fir_plot_{i}.png", buf.getvalue(), "image/png")
        set_status("Exported individual plot PNGs.")
        return

    # Combine all figures vertically into one PNG
    images = []
...
# ========================
def _bind(el_id: str, event: str, func):
    """Bind a DOM event with a stable JS proxy (prevents GC)."""
    proxy = create_proxy(func)
    document.getElementById(el_id).addEventListener(event, proxy)
    _proxies.append(proxy)


async def initialize_app():
    # Ensure window options exist (in case HTML was edited)
    try:
        wsel = document.getElementById("wtype")
        if wsel.length == 0:  # if no options present, populate from map
    ...
    # Bind events (using stable proxies)
    _bind("designBtn", "click", design)
    _bind("resetBtn", "click", reset_defaults)
    _bind("downloadCSV", "click", export_csv)
    _bind("downloadPlot", "click", export_plots)
    _bind("wtype", "change", update_tooltip)

    # Defaults (no auto-design to keep first experience clean)
    reset_defaults()
